...
void writeEEPROM(uint32_t addr, const uint8_t *data, int bytes) 
{
  ...
  waitEEPROM(addr);
  boolean fail = I2c.write(  (uint8_t) EEPROM_I2C_ADDR + (uint8_t) ((addr>>16) & 0x7), 
...
boolean readEEPROM(uint32_t addr, uint8_t *data, int size) 
{
  ...
  boolean fail = I2c.read((uint8_t) EEPROM_I2C_ADDR + (uint8_t) ((addr>>16) & 0x7), (uint16_t) (addr & 0xFFFFL), data, size) != 0;
